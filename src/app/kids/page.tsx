...
const ALL_SCHOOLS = [
  ...
  { name: '강남초등학교', addr: '서울 강남구 역삼동', reports: 0, blind: 1, safe: 94, reportColor: 'safe', blindColor: 'warn', safeColor: 'safe' },
  { name: '둔전초등학교', addr: '경기 성남시 수정구', reports: 2, blind: 1, safe: 89, reportColor: 'danger', blindColor: 'warn', safeColor: 'safe' },
  { name: '송파초등학교', addr: '서울 송파구 가락동', reports: 1, blind: 0, safe: 97, reportColor: 'warn', blindColor: 'safe', safeColor: 'safe' },
]
...
export default function KidsPage() {
  ...
  const schools = query.trim()
    ? ALL_SCHOOLS.filter(s => s.name.includes(query) || s.addr.includes(query))
    : ALL_SCHOOLS
  ...
  return (
    <div className="max-w-lg mx-auto">
      ...
      <div className="px-4 py-3 flex gap-2">
        <input
          className="flex-1 px-3 py-2.5 rounded-xl text-[13.5px] outline-none border"
          style={{ borderColor: 'var(--border)', background: 'var(--white)' }}
          placeholder="학교 이름 또는 주소 검색"
          value={query}
          onChange={e => setQuery(e.target.value)}
        />
        <button className="px-4 py-2.5 rounded-xl text-white text-[13px] font-bold flex items-center gap-1"
      ...
      <p className="px-4 text-[10.5px] font-black tracking-widest uppercase flex items-center gap-1.5 mb-2" style={{ color: 'var(--p600)' }}>
        <Baby size={13} /> 주변 학교 현황 {query && <span className="font-normal normal-case ml-1">— &quot;{query}&quot; 검색 결과 {schools.length}건</span>}
      </p>

      {schools.length === 0 ? (
        <div className="mx-4 bg-white rounded-2xl p-6 text-center border" style={{ borderColor: 'var(--border)' }}>
          <p className="text-[14px] font-bold mb-1" style={{ color: 'var(--text)' }}>검색 결과 없음</p>
          <p className="text-[12.5px]" style={{ color: 'var(--text3)' }}>다른 학교명이나 주소로 검색해보세요.</p>
        </div>
      ) : (
